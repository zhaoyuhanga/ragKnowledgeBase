...
│
├── rag-qa-frontend/            # 前端项目（React 18 + TypeScript + Vite + Ant Design 5）
│   ├── src/
│   │   ├── pages/            # 页面组件（Dashboard/Documents/QA/RetrievalTest…）
│   │   ├── components/       # 公共组件（Header/Sidebar/Layout/Stats）
│   │   ├── services/         # API 调用封装（按后端模块一对一）
│   │   ├── types/            # TypeScript 类型定义
│   │   └── styles/           # 全局样式
│   ├── index.html
│   ├── package.json
│   ├── tsconfig.json
│   └── vite.config.ts        # 开发端口 3000，/api 代理到 8011
│
├── scripts/                    # 启动脚本
│   ├── start-all-local.bat
│   ├── start-all-local.sh
